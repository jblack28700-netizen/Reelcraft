# Reelcraft headless backend (Decision 059, Slice 1).
#
# Builds the same application sources as reelcraft.pro EXCEPT app/ui/, and links
# the Qt HTTP server instead of QtWidgets. This is an ADDITIVE target: it does not
# replace or modify reelcraft.pro or tests/tests.pro.
#
# Build out-of-source so the root Makefile (the GUI application) is left alone:
#
#     mkdir -p build_server && cd build_server
#     /usr/lib/qt6/bin/qmake QMAKE_CC=/usr/bin/gcc QMAKE_CXX=/usr/bin/g++ ../reelcraft_server.pro
#     make
#
# Run:
#     ./reelcraft_server --media <path-to-360-clip> [--host 127.0.0.1] [--port 8732]

QT += httpserver core gui concurrent
INCLUDEPATH += app
CONFIG += c++17
TEMPLATE = app
TARGET = reelcraft_server

SOURCES += \
    server/main.cpp \
    server/Worker.cpp \
    server/JobRegistry.cpp \
...
RESOURCES += server/resources.qrc
