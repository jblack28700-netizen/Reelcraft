...
## 2026-09-20 — Environment Restoration: Qt 6 Toolchain Reinstalled and the Objective 41 Baseline Re-Verified

### Objective

The container that produced Objective 41 was replaced. Its rootfs had no Qt at all, so nothing in the
project could be compiled or run in it, and the recorded baseline was not reproducible. Restore the Qt 6
development toolchain, rebuild from source with it, and prove it reproduces the Objective 41 baseline
before any further work — full-suite validation, no partial run.

### What inspection established first

- The Objective 41 binaries and objects survived on the persisted `/workspace` volume and were linked
  against `libQt6Widgets/Gui/Core/Test.so.6` at 04:04-04:05, minutes before the 04:07:47 commit; the
  current rootfs was created at 13:05 and its dpkg log held no qt6 entries. So Qt was present in the
  build environment at Objective 41 and absent here — a container replacement, not a repository change.
- A change impact worth recording: after reinstalling Qt and running `qmake`, a plain `make` reported
  *"Nothing to be done for 'first'"*. The git-ignored objects from the previous container are newer than
  the sources, so the tree looked current and **nothing would have been recompiled**. Running
  `scripts/build_and_test.sh` in that state would have exercised the old binaries against the new
  libraries and proved nothing about the toolchain, so both trees were cleaned and fully rebuilt.

### What was done

- `apt-get update` (the apt index was empty).
- `apt-get install -y --no-install-recommends qt6-base-dev qt6-base-dev-tools` — see the note below on
  why Recommends were skipped.
- `qmake QMAKE_CC=/usr/bin/gcc QMAKE_CXX=/usr/bin/g++` in both `reelcraft/` and `tests/`, using the
  documented invocation.
- `make clean` in both trees, then a full serial `make` in each, then
  `scripts/build_and_test.sh` to completion.

### Verification

- **Installed toolchain**: `qt6-base-dev` and `qt6-base-dev-tools` at `6.4.2+dfsg-21.1build5`;
  `qmake -v` reports *QMake version 3.1 / Using Qt version 6.4.2 in /usr/lib/x86_64-linux-gnu*. The
  Objective 41 Makefiles record *Generated by qmake (3.1) (Qt 6.4.2)* — **the same Qt version**, so the
  toolchain is a reinstallation of the recorded one rather than a different one. Compiler is gcc 13.3.0
  (the environment doc records 13.2.0 at Objective 41; a point-release difference).
- **Rebuild**: 62 objects in each tree, both binaries relinked against the restored Qt, no build errors.
  `make` root ~117 s, `make` tests ~165 s (including the moc pass over the 24k-line test TU).
- **Full suite**: `scripts/build_and_test.sh` exited **0** in ~80 s with
  `Totals: 530 passed, 0 failed, 14 skipped, 0 blacklisted, 79853ms` — **exactly the Objective 41
  baseline**, including the same 14 by-design skips.
- **Checkpoint**: `scripts/checkpoint_check.sh` reports **PASS** (exit 0). Both build-tree freshness
  checks now pass (they previously failed for the missing-toolchain reason), the suite timeout is
  consistent, and 59 decisions are present.
- **Scope check**: 0 lines deleted from `DECISIONS.md`; no file under `app/`, `tests/`, `scripts/`,
  `tools/` or any `.pro`/`Makefile` was modified.

### Deviation recorded: `--no-install-recommends`

The install was run with `--no-install-recommends` rather than the plain command. A simulation showed
that skipping Recommends drops exactly 13 optional packages (QML, Quick, Wayland client/compositor/EGL/
wl-shell, SQL sqlite, the GTK platform theme and translations) and keeps every hard dependency —
including `qt6-qpa-plugins`, which is a **Depends** and supplies the offscreen platform plugin the test
suite requires. That is the policy the objective asked for (do not install optional Qt modules not
required by the `.pro` files) while preserving everything the build and the offscreen suite need. No
QtWebEngine, QtMultimedia or `qt6-base-private-dev` was installed.

### Boundary notes / not implemented

- Documentation only beyond the toolchain: no source, test, script, `.pro` or Makefile change, and
  `DECISIONS.md` untouched.
- No push: no git credential is present in this container (no private key, no agent, no helper, no
  token). The Decision 059 commit therefore remains local and unpushed.
- The baseline was re-confirmed and unchanged; no test, baseline or code adjustment was made.

### Decisions

- None required. Decisions 001-059 preserved.
