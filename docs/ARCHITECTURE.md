...
| Creator revision (record level) | `Application::{revisionOutputPath,reviseReframeOutput,revisionsOf}` over the Objective 17 mechanism | revision = a NEW immutable decision whose single parent is the record it revises; destination is a fresh `<base>_reframe_rev<N>.mp4` sibling (never an existing file, never a held record's path); supersession is DERIVED from lineage at read time, never stored | `main.cpp` wiring, UI |
