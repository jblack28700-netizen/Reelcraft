...
A Qt Test suite covers project state, viewer/media, the media-source seam and frame pump, player/timing, the 360 reframing engine (plan validation/round-trip, camera interpolation, rendering determinism, intent parsing, plan building, a real FFmpeg end-to-end render, and — Objective 28 — source-audio preservation over the plan's retained spans, its degradation semantics and its replay equivalence), and target resolution (equirect/view geometry, seam and pitch boundaries, view coverage, deterministic tracking, resolver behavior, the subprocess detector protocol, the track planner, and a model-free detection -> plan -> render path). Current result: **466 passed, 0 failed, 9 skipped** (measured at the Objective 28 checkpoint; the real-FFmpeg render, decode and analysis tests dominate, and the wall time varies widely with device and load — the same suite has been recorded at ~732 s on the proot development device and at ~60 s on a native container). Eight skips are environment-gated real-media/model integrations (real detector, speaker, appearance, real 360 clip, rendered playback, source playback, compound command, application/user command) that run only when the corresponding helper, model and clip variables are configured; the ninth is the child-only slot driven by the fresh-process replay test. The normal suite stays model-free.
