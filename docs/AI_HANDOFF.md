...
A further distinction now applies. **Objective 18 (Deterministic Intent -> Plan Contract Checker) is formally SCOPED but NOT implemented** (Decision 035). Its scope, rule set, exceptions and exclusions are recorded; no code exists and implementation is not authorized. Do not treat the recorded scope as a description of existing behaviour.
