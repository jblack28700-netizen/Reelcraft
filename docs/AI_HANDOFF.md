...
The project is far past the documentation-foundation stage: a working Qt 6 desktop shell, project and
media model, deterministic review and playback paths, a Phase 3 media-engine foundation, and a
Phase 4 360 reframing engine all exist and are verified. **Read `CURRENT_STATE.md` for what exists
now, `NEXT_TASK.md` for the capability register, validation debt and candidates, and
`ARCHITECTURE.md` §23-24 for the seam and behaviour-to-test indexes.** Do not assume planned
architecture is implemented, and do not re-derive status from this file: it is orientation, not the
record.

**Priority.** A working **360 reframing/editing capability**: 360 source -> scene/subject
understanding -> natural-language or structured request -> structured edit/reframe plan ->
virtual-camera decisions -> deterministic execution -> flat video output.

**Orientation to the 360 pipeline.** `ReframeIntent` (deterministic parser: directions, time ranges,
subject references, temporal edits, compound clauses, lens control, plural subject groups) ->
`TargetResolver`/`TargetSelector`/`TargetIdentity` (replaceable detector seam, spherical tracking,
identity and selection) -> `ReframePlanBuilder` or `TargetTrackPlanner` (plan, follow path,
multi-subject enclosure) -> `ReframeContract` (IPC-1..4) -> `ReframeRenderer`/`ReframePipeline`
(deterministic frames, H.264, optional source audio) -> `EditDecision` (persisted, versioned,
replayable without perception). The composition entry point is `ReframeCommandRunner`; the
application owns inputs, persistence and UI.

**Rules that are stop-and-report conditions when they would be broken.**

- **Analysis is evidence, never an editorial decision** — no plan, cut, keyframe or A-roll label in
  `MediaAnalysis`; if two creators could disagree about a statement, it is reasoning, not analysis
  (Decision 039).
- **Analysis is never on the deterministic replay path** — `replayEditDecision()` consumes
  `EditDecision::plan()` and nothing else; deleting every analysis artifact must change nothing
  (Decision 040).
- **A capability that cannot run is `Unavailable` with a reason** — never an empty success;
  `Unavailable`, `Failed` and an empty layer are three different states (Decision 041).
- **Analysis is derived data owned outside the project file** — the project stores a small
  `analysisRefs` reference only; the project schema stays 3 (Decisions 038/041).
- **Persisted decisions are immutable** — a revision is a new decision with a single parent hash,
  never a mutation (Decisions 033/034).
- **Original media is read-only** on every path, and the source-reference vocabulary is shared
  (`core/MediaSourceReference.*`), never duplicated per artifact.
- **Do not silently clamp or fabricate.** An unsatisfiable request is refused with the measured
  reason (target direction, lens, multi-subject containment); an unresolved subject is reported, not
  guessed.

**Known traps — each is recorded in its decision; do not "improve" them by intuition.** Do not fix
the covering-view duplicate finding by raising the tracker merge distance (Decision 047). Keep the
framing phrase consumed before direction detection is consulted, or `pull back`/`back up` turn the
camera while widening the lens (Decision 049). Do not smooth the multi-subject path with the
single-track smoothing — it can move the camera off the framing that guarantees containment — and do
not add a cosmetic IPC-5: the plan retains camera coordinates only, so multi-subject containment is
not decidable in the contract (Decision 050). Do not treat in-app playback as audio-capable: it is
video-only (Decision 036).

**Deliberately NOT built, and not to be assumed:** transcript/dialogue, diarization, scene or shot
segmentation, quality/salience metrics, B-roll reasoning, an LLM reasoner, Creator Memory,
embeddings, generated coverage, multi-source editing, timeline/transition/caption/colour features,
equirect export, and an in-app audio output subsystem. Each needs its own approved objective.

**Build-integrity rule (learned during Objective 20).** `reelcraft/` and `tests/` each have their
own generated `Makefile`; after adding, renaming or removing source or header files, run `qmake` in
**both** directories and `make` twice, so a stale dependency list cannot link two revisions of a
class. `scripts/checkpoint_check.sh` verifies both trees are current.
