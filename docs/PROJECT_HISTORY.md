...
## 2026-09-18 — 360 Reframing Objective 23: Subject-Follow Camera Paths

Reelcraft closed the gap between "the pipeline runs" and "the pipeline does the thing the product is for". Every stage of the 360 path already existed and was tested — natural-language instruction to structured intent, intent to resolved subject, resolved subject to a validated plan, plan to a deterministic render — and yet the flagship instruction class did not work: because the plan builder emitted exactly one camera keyframe per camera move from a single resolved direction, "follow me" and "keep me centered" produced a locked-off shot aimed at wherever the subject happened to be, for the entire clip. The component that would have fixed it already existed. TargetTrackPlanner turned a tracked subject's time-ordered observations into a real camera path, it was unit-tested, and no production code ever called it: the trajectory was being computed and thrown away at the last step. Objective 23 (Decision 043) connected it. The intent now records whether a clause asks for continuous framing or a one-shot aim, using the same pattern table that extracts the subject so the two can never disagree, and the command runner builds a camera path through the resolved track for a follow instruction. The scope is deliberately narrow: only a single target-referencing follow move takes the new path, the ordinary builder still runs first and remains both the validity gate and the fallback, and aim instructions such as "look at the car" deliberately stay a single fixed direction. Nothing about the plan, camera path, renderer, playback, replay or persisted decisions changed, so determinism, immutability and perception-free replay are untouched. The work also produced an honest finding it did not act on: chasing a failing real-media test through three eliminated hypotheses ended at the covering-view detector reporting one subject from two overlapping views with centroids 9.6 degrees apart — more than the tracker's person-tuned 8-degree merge distance — which left two identities and made the reference legitimately ambiguous. That is perception tuning in Decision 019 territory, left alone with the evidence recorded. Four new tests, including a real encoded 360 clip in which the subject walks across the sphere and the resulting render is decoded back, and a targeted regression of 45 tests with no failures. Decisions 017-042 are preserved.


## 2026-09-18 — 360 Reframing Objective 24: Follow Trajectory Sampling Density

Reelcraft closed the gap between "the camera follows the subject" and "the camera follows the subject smoothly". Objective 23 had connected the resolved trajectory to the camera path, but the trajectory itself was still sampled with a budget shared by every subject command — five timestamps across the whole instruction — so a follow path could only ever move in five steps however long the clip was. Five samples is exactly right for aiming at someone, where only a representative direction matters, and far too sparse for tracking them. Objective 24 (Decision 044) separated those two cases. A follow command now samples its range at a resolution of its own, expressed as a time interval rather than a count so that a long clip does not silently degrade into coarse steps, bounded by a maximum so the decode and detection cost stays finite: seventeen keyframes at quarter-second spacing over the four-second fixture that previously produced five at one-second spacing, and a hard ceiling of twenty-four samples per follow command. The measurement that justifies the choice is that the trajectory does not change — the subject's eighty-degree sweep comes out as the same 78.7 degrees at every density, only resolved more finely — and that density maps one-for-one into camera keyframes, so the mechanism is doing exactly one thing. Aim, direction and speaker commands keep the original five-sample budget and their existing behaviour and cost, and an explicit caller-supplied timestamp list still overrides both. A second, unplanned measurement came out of a failing assertion: three timestamps spaced two seconds apart move the subject forty degrees per step, past the tracker's association gate, so the subject splits into separate identities and the reference honestly resolves as ambiguous, while the same trajectory sampled twice as finely associates cleanly. Sampling density turns out not to be only a smoothing concern but an association one. Nothing about perception changed: no threshold, detector, cover-view association, identity semantic, planner, renderer or persisted schema was touched, and the duplicate-identity finding from Objective 23 is preserved untouched for its own investigation. One new test measures five densities; the targeted regression returned 46 passing tests with no failures. Decisions 017-043 are preserved.


## 2026-09-18 — 360 Reframing Objective 25: Persistent Decoder Reuse for Trajectory Resolution

Reelcraft stopped paying for a fresh decoder on every trajectory sample. Objective 24 had deliberately made follow commands sample a subject's movement up to twenty-four times, which made the camera move smoothly but also made resolution open up to twenty-four FFmpeg processes, one per sample, because each frame request went through an extraction path that builds and tears down its own process. The measurement that made this worth fixing was not about pixels at all: on this device a single-frame decode of a tiny 360x180 clip costs roughly the same as one of a 4K source, about two and a half seconds, because what dominates is process start-up and the syscall cost of the proot container rather than the decoding. Process count, not resolution, was the lever. The fix was to change which implementation sits behind the resolution seam rather than to touch the seam itself: the default trajectory-resolution provider is now the persistent streaming provider the render path has used since Objective 20, built the same way, from a frame rate read once through the existing ffprobe seam. It keeps one stream open at an anchor, reads forward, re-anchors when a request moves beyond its bounded window, and falls back to the positioned seek whenever it cannot stream — so an unknown frame rate, a failed stream or the end of the media degrades to exactly the previous behaviour rather than to a different frame. Correctness was inherited rather than assumed: Objective 20 had already proved that provider's frames byte-identical to the seek path's, and the new test asserts the strongest available form of that at the resolution level, that the same timestamps produce the same observations, the same keyframe count and exactly equal keyframe timestamps and angles. Measured with every FFmpeg invocation counted through a recording wrapper, six samples went from six processes and fifteen seconds to three processes and ten, and the real-media follow test fell from thirty-eight seconds to twenty-seven. Nothing about perception, thresholds, identity resolution, the planner, the camera path, the renderer, the parser or any persisted schema changed, no new seam or dependency was introduced, and the duplicate-identity finding from Objective 23 remains preserved for its own investigation. One new test; the targeted regression returned 51 passing tests with no failures. Decisions 017-044 are preserved.


## 2026-09-18 — 360 Reframing Objective 26: Deterministic Follow Trajectory Smoothing

Reelcraft made its follow camera move like a camera rather than like a list of detections. Objectives 23 to 25 had made the follow path possible, dense and cheap, but its quality as movement had never been addressed: the planner emitted one camera keyframe per resolved observation carrying the detected direction verbatim, and the renderer interpolates linearly between keyframes, so the path was piecewise linear through the raw detections — its velocity broke at every keyframe and every sample-to-sample wobble of the detector went straight into the camera. Objective 26 (Decision 046) added a deterministic, model-free smoothing step inside the trajectory planner, chosen because inspection showed that planner is called by the follow path alone and that trajectory shaping is precisely its stated job. The mechanism is a centred moving average whose window shrinks symmetrically at the ends, and that detail is what makes it safe rather than merely plausible: because the average is never taken over a one-sided neighbourhood the path is not delayed and constant-velocity motion is reproduced exactly, so smoothing removes jitter without flattening real movement; because every smoothed value lies inside the range of the values it averaged from it cannot overshoot; and because keyframe times, count and ordering are never touched, start and end timing, direction and total span survive by construction. Yaw is unwrapped into a continuous angle before averaging, so a subject walking across the plus-or-minus-180-degree boundary is followed the short way round instead of folding to zero. On a synthetic wobble the largest step between consecutive keyframes fell from forty degrees to four, while a linear ramp came through unchanged and a stationary subject stayed exactly still. Framing was investigated and deliberately left centred, because "follow the person" and "keep me centered" mean centred, and inventing a lead-room offset would have quietly changed what those commands ask for. Nothing about detection, identity, sampling density, decoding, the renderer, the parser or any stored edit changed, aim and explicit-direction and speaker and multi-move commands never reach the planner and are untouched, and the cost is negligible because the layer works only on keyframes that were already resolved. Four new tests; the targeted regression returned 62 passing tests with no failures. Decisions 017-045 are preserved.



## 2026-09-18 — 360 Reframing Objective 27: Covering-View Duplicate Identity

Reelcraft fixed the one perception defect its own follow work had kept finding and refusing to guess about. Objective 23 had observed that a single subject could be reported by two overlapping covering views with centroids 9.6 degrees apart — further than the tracker's person-tuned 8-degree merge distance — leaving two identities and making every reference to that subject honestly resolve as ambiguous; Objectives 24, 25 and 26 each recorded the finding as deliberately untouched, waiting for evidence rather than a threshold tweak. Objective 27 (Decision 047) produced that evidence by reading the raw per-view detections instead of the merged result. The covering plan overlaps its views on purpose, so a subject near a view boundary is always seen twice, and the neighbour sees a clipped silhouette whose box centre is dragged toward that neighbour's own axis: in the reproduction one report carried an 11.68-degree yaw half-extent and the other 1.35 degrees, with 9.61 degrees between their centres. That displacement is unbounded — it grows with the subject's apparent size and shrinks with its distance from the boundary — so no fixed merge distance can absorb it without also asserting something false about how close two people can stand. The fix therefore uses information the observation already carried and the merge test was ignoring: a detection reports how large it looks, so duplicates are now consolidated when their centroids are within the merge distance **or** when their reported yaw footprints reach each other, which is exactly the condition under which a clipped report is evidence of the same object rather than a small distant one. The global threshold was deliberately not raised; the 24-degree overrides that four earlier tests had been carrying as workarounds were deleted, so the shipped default is what the follow, sampling, decoder-reuse, smoothing and real-media pipeline tests now exercise. The risk the objective was really about was over-merging, and it is asserted directly: two subjects twelve degrees apart stay two identities, two subjects far apart stay two, and a single subject stays one, while a contrast case raising the threshold to 24 degrees shows it merging a genuinely separate pair — the behaviour that made the threshold route unacceptable. Determinism is unchanged and re-checked on a freshly constructed resolver, and nothing else moved: no coverage, projection, detector, gate, identity, sampling, smoothing, renderer, decoding or persisted-schema change, no new dependency and no model. Three new tests; the targeted regression returned 73 passing tests with no failures. Decisions 017-046 are preserved, and the finding those decisions recorded as open is closed.



## 2026-09-18 — 360 Reframing Objective 28: Rendered Output Preserves Source Audio

Reelcraft's reframing engine stopped handing back silent pictures. The project's priority pipeline is *360 source -> scene and subject understanding -> natural-language or structured request -> structured edit/reframe plan -> virtual-camera decisions -> deterministic execution -> flat video output*, and every stage of it was implemented and tested except the completeness of the last: the encoder accepted a rendered-PNG pattern and nothing else, so it wrote H.264 video with no audio input, no audio map and no audio codec, the pipeline never handed it the source path, and the decode seams explicitly discard audio. No test observed the absence, because every fixture the suite generates is video-only -- the gap was invisible precisely because it was universal. Objective 14 had already turned that silence from an omission into a defect: retained `segments` move a render onto a different timeline from its source, so the sound of a retimed result cannot simply be copied across; it has to be mapped through the same spans the picture was built from. Objective 28 (Decision 048) does exactly that, and does it without touching a single persisted artifact. The audio is taken over the plan's retained source spans -- its ordered segments when it has them, otherwise its source range -- each span trimmed out of the source, its timestamps reset, and the spans concatenated in order, so the sound begins at output time zero and matches the picture that was kept. Deciding *whether* there is audio to preserve was kept as a fact reported through the existing `MediaDurationProbe` seam rather than a guess, which is what gives the feature honest edges: footage with no sound renders exactly the silent output it always did and reports nothing, a probe that cannot answer renders that same output and records why, and audio that exists but cannot be produced is a hard failure instead of a quietly silent file. The design choice that carries the most weight is that the picture keeps its existing encoder: pass one produces the video-only file through the unchanged path and pass two remuxes that stream with `-c:v copy` beside the new audio, so the video in an output that carries sound is not merely equal to the previous output's video, it *is* the previous encoder's output. That is why a video-only source still renders a container byte-identical to the standalone encoder's, why the streaming-versus-seek equivalence test needed no relaxation, and why the change can be reasoned about as strictly additive. Two smaller measurements shaped the rest: a retained span's frame count rounds down, so an unbounded concatenation left a 3000 ms container for 2000 ms of picture until the audio was bounded to `frameCount/fps`; and the source's channel layout and sample rate are preserved from reported facts, with the channel count forced only for mono and stereo so an uncommon layout is preserved by the muxer rather than approximated -- spatial audio, accordingly, is carried as-is and not rotated by the virtual camera, which is recorded as a limitation rather than smoothed over. Nothing persisted changed: no plan, keyframe, decision or project schema, and no new field, because the audio is a pure function of the source and the plan and therefore comes back through the existing perception-free replay. Seven new model-free tests verify it by *where the signal is* -- generated fixtures whose audio is a tone except inside one silent window -- covering a continuous range, disjoint retained segments in order, a late and a rounded range, a video-only source asserted byte-identical to the old path, an unusable probe degrading with a recorded reason, a stereo source with its fingerprint and content digest unchanged, and an application command whose replay reproduces both decoded picture and decoded audio. Targeted regression returned 20 passed / 0 failed / 0 skipped and the full model-free suite 466 passed / 0 failed / 9 skipped. What remains is not a rendering problem: Reelcraft still cannot play sound, so the creator can produce a complete result and watch it in an external player but cannot hear it inside the application -- the audio output subsystem Decision 036 deferred, which needs its own objective and its own dependency decision. Decisions 017-047 are preserved; Decision 048 records this one.



## 2026-09-18 — 360 Reframing Objective 29: Natural-Language Framing (Lens) Control

Reelcraft's virtual camera learned to change its lens. Everything the 360 reframing engine could do until this objective was about *where* the camera looked: it could aim at a named subject, follow a resolved subject over time with a dense, smoothed trajectory, retime the source into retained segments, and deliver the retained audio with the picture. It could not change how *close* the shot was. The plan builder wrote a field of view of ninety degrees into every keyframe it produced, and the two specialist planners carried their own constants, so a creator asking to "zoom in" got, at best, the same framing with a note saying the instruction had not been recognised. That was the wrong shape of gap for a product whose central promise is automated reframing from natural language, because tightness is half of what a framing request means: a wide establishing view and a close-up on the speaker are the same camera position and a different lens. Objective 29 (Decision 049) closed it without adding anything to the architecture's persistent surface, because the executable representation already had the field: a camera keyframe has always carried its field of view, the camera path has always interpolated it, the renderer has always honoured it, and the plan validator has always bounded it. What was missing was a decision layer that could set it. A framing clause is now parsed as a lens request with a deterministic vocabulary — named absolute levels checked most specific first ("zoom in" and "close-up" at sixty degrees, "extreme close-up" and "zoom in a lot" at forty, "wide", "zoom out" and "pull back" at one hundred and twenty, "very wide" and "zoom all the way out" at one hundred and forty), explicit numbers ("field of view 60", "60 degree field of view", "fov=75"), and a softener that halves the step toward ninety for "slightly closer". Two traps found by reading the parser rather than by testing shaped the rest: the direction parser reads "back" as a half-turn, so the natural widening phrases would have both turned the camera and widened the lens until the consumed framing phrase was removed from the text the direction parser sees — the same technique the compound-command work established for temporal ranges — and the subject capture is end-anchored and greedy, so "follow me and zoom in" would have made "me and zoom in" the subject unless a trailing framing clause is stripped from a reference exactly as a trailing temporal word already was. Framing therefore composes: one clause carrying framing plus a direction or a subject is one move carrying both, a framing-only clause becomes a real move that changes the lens while holding the camera still, and the lens persists until it is changed again, which makes "start wide, then push in on me" a genuine two-keyframe move rather than a dropped instruction. Every execution path was wired explicitly instead of assumed — the builder per move, the follow path through the trajectory planner's configuration, the speaker path through its own — and a lens *change* on the speaker path, which that planner cannot express, is refused honestly rather than rendered at one lens. A fourth contract rule, IPC-4, now guarantees that a requested lens is actually reached by the executable plan, deliberately as containment of the request rather than equality over keyframes because a lens change legitimately starts from the lens the camera already had; an unsatisfiable lens is reported and refused rather than silently clamped, and the framing that was applied is reported with the result so a creator sees it. Six new model-free tests cover the vocabulary, the builder's lens state, the contract rule, the follow path (same trajectory, different lens), the speaker path's honest refusal, and an end-to-end render whose picture must change at a tighter lens together with an Application command whose stored decision carries the lens and whose replay reproduces the frames. Targeted regression returned 73 passed / 0 failed / 0 skipped and the official full suite 472 passed / 0 failed / 9 skipped. Decisions 017-048 are preserved; Decision 049 records this one.



## 2026-09-18 — 360 Reframing Objective 30: Multi-Subject Framing

Reelcraft's camera learned to frame two people at once. Every reframing decision the engine could express until this objective aimed at one subject: the plan builder resolved a single direction per camera move, and the follow work of Objectives 23 to 27 built a trajectory through a single track. "keep both of us in frame" had no representation anywhere in the system — the parser found neither a direction nor a subject in it, so the instruction was dropped as unrecognized — even though the two things it needs had existed for a long time: a camera keyframe that can carry any aim and any lens, and target tracks that report not only where every subject is but how large it appears. Objective 30 (Decision 050) joined them. Inspection before implementation answered the three questions that decided the design, and the answers are why this objective is small rather than architectural: the executable representation needed no new field at all, because a framing that contains two subjects is simply a keyframe aimed between them at a lens wide enough for both, and the camera path already interpolates aim and lens together; perception needed no new provider, because the resolver already returns every track, each observation already carries its angular footprint, and the selector already resolves the creator, the one other person, and the canonical two-person order deterministically while reporting ambiguity instead of guessing; and the framing mathematics has an exact rather than an invented answer, because the renderer's own camera basis says precisely when a subject is inside the frame, and inverting it conservatively can never under-frame. A plural request is therefore recorded as a GROUP rather than as tracks — "both of us" meaning the creator and the one other visible person, "both people" meaning exactly two visible people — and is resolved at command time against whatever identities actually exist, so nothing is fabricated at parse time and nothing is persisted. The camera aims at the midpoint of the two footprints, which privileges neither person, and the lens is the tightest that contains both for the whole instruction, so a pair that walks apart is followed at the width their widest moment requires. When the two cannot share one frame, or when a named lens such as a close-up is too narrow to contain them, the command says so and reports what would be needed rather than quietly framing one of them or widening the shot that was asked for; the same honesty covers a subject that left the range, two subjects never seen together, a creator who was never selected, three visible people when exactly two were requested, and a plural request mixed with a direction. One boundary was recorded rather than papered over: the contract checker cannot verify this, because by Decision 035 the executable plan retains camera coordinates only and therefore says nothing about who is in frame — a rule asserting merely that two identities were resolved would prove nothing, so the guarantee lives where the identities and footprints exist, in the command runner before any plan is built, and the tests assert the exact containment condition at every keyframe using the subjects' real positions. Six new model-free tests cover the vocabulary, the mathematics, the two-identity resolution, every refusal, and a moving pair whose plan is persisted in an edit decision and replayed to identical decoded frames with the source untouched. Targeted regression returned 102 passed / 0 failed / 0 skipped and the official full suite 478 passed / 0 failed / 9 skipped, exactly six more than the previous checkpoint. Decisions 017-049 are preserved; Decision 050 records this one.
